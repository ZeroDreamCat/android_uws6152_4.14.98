 ...
 * MicroArray Fprint Driver Code for REE environment
 * madev.c
 * Date: 2017-3-15
 * Version: v4.0.06
 * Author: guq
 ...
 * 
 * Ported to SPRD SharkL5 platform
 */
#include "madev.h"
// 移除不存在的头文件，避免编译错误
// #include <fp_vendor.h>
...
int mas_sync(u8 *txb, u8 *rxb, int len) {
	...
	mutex_lock(&dev_lock);
    mas_select_transfer(smas->spi, len); // 紫光上此函数为空，不影响
...
static long mas_ioctl(struct file *filp, unsigned int cmd, unsigned long arg) {
    ...
    switch(cmd){
        ...
        case WAKEUP:                                                       //wake up, schedule the process into the runqueue
            ...
            break;
        case ENABLE_CLK:
            mas_enable_spi_clock(smas->spi);                                    //紫光上为空操作
            break;
        case DISABLE_CLK:
            mas_disable_spi_clock(smas->spi);                                   //紫光上为空操作
            break;
...
static ssize_t mas_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos) {
    ...
    if(count==6) {                                                                              //cmd ioctl, old version used the write interface to do ioctl, this is only for the old version
        ...
        arg += tmp[5];
#ifdef COMPATIBLE_VERSION3
        // 修复：移除多余的 NULL 参数，与 version3_ioctl 声明匹配
        val = version3_ioctl((unsigned int)cmd, (unsigned long)arg);
#endif
	} else {        
        //memset(stxb, 0, FBUF);
...
int mas_mmap(struct file *filp, struct vm_area_struct *vma)
{
    ...
    if (size > 512 * 1024) {
        MALOGE("mmap size too large: %lu\n", size);
        return -EINVAL;
    }

    if (kernel_memaddr) {
        /* 已分配过，检查请求大小不能超过已分配大小 */
        if (size > kernel_memesize) {
            MALOGE("mmap request size %lu exceeds allocated %lu\n",
                   size, kernel_memesize);
            return -EINVAL;
        }
    } else {
        /* 首次分配 */
        order = get_order(size);
        kernel_memaddr = (void *)__get_free_pages(GFP_KERNEL, order);
        if (!kernel_memaddr) {
            MALOGE("Failed to allocate pages for mmap (order=%u)\n", order);
            return -ENOMEM;
        }
        kernel_memesize = PAGE_SIZE << order;
        memset(kernel_memaddr, 0, kernel_memesize);
        printk(KERN_INFO "mafp: mmap allocated %lu bytes at vaddr=%p\n",
               kernel_memesize, kernel_memaddr);
    }

    pfn = virt_to_phys(kernel_memaddr) >> PAGE_SHIFT;
    vma->vm_page_prot = pgprot_noncached(vma->vm_page_prot);

    if (remap_pfn_range(vma, vma->vm_start, pfn, size, vma->vm_page_prot)) {
        MALOGE("remap_pfn_range failed (pfn=%lu, size=%lu)\n", pfn, size);
        return -EAGAIN;
    }

    vma->vm_flags |= VM_DONTEXPAND | VM_DONTDUMP;
    printk(KERN_INFO "mafp: remap_pfn_range pfn:[%lu] size:%lu ok.\n", pfn, size);
    return 0;
}
...
static int init_interrupt(void)
{
    ...
    irq = mas_get_irq(&smas->spi->dev);   // 改动：从 spi->dev 获取中断号
    if(irq<=0){
        ret = irq;
        MALOGE("mas_get_irq failed, irq=%d", irq);
        return ret;
    }

    /* 强制设置中断触发类型为上升沿，因为设备树中 flags 为 0 */
...
#endif
    if (ret < 0) {
        MALOGE("request_irq failed, irq=%d, ret=%d", irq, ret);
        return ret;   // 直接返回，不执行 enable_irq_wake
    }
    enable_irq_wake(irq);
    return 0;
}
...
static int init_vars(void)
{
    ...
    /* 修复：memset 长度应为实际字节数 IMAGE_DMA_SIZE，而非 get_order() */
    memset(stxb, 0, IMAGE_DMA_SIZE);
    memset(srxb, 0, IMAGE_DMA_SIZE);
    ...
    if (!gWorkq) {
        ...
        if (sdev) kfree(sdev);
        if (smas) kfree(smas);
        if (stxb) free_pages((unsigned long)stxb, get_order(IMAGE_DMA_SIZE));
        if (srxb) free_pages((unsigned long)srxb, get_order(IMAGE_DMA_SIZE));
        return -ENOMEM;
    }
    return 0;
}
static int deinit_vars(void)
{
...
#else
    /* 4.14+ 内核已不再使用 wake_lock，统一用 wakeup_source */
    wakeup_source_trash(&gProcessWakeLock);
#endif

    if (smas && smas->input) {
        input_unregister_device(smas->input);
        input_free_device(smas->input);
    }
    ...
    if (smas)
        kfree(smas);
    /* 修复 mmap 内存释放：必须用 free_pages，不能用 kfree */
    if (kernel_memaddr) {
        free_pages((unsigned long)kernel_memaddr, get_order(kernel_memesize));
        kernel_memaddr = NULL;
        kernel_memesize = 0;
    }
...
static int init_notifier_call(void)
{
    ...
    err = fb_register_client(&notifier);
    if (err)
        return err;
    is_screen_on = 1;
    return 0;
...
int mas_plat_probe(struct platform_device *pdev) {
    // 改动：紫光不使用 platform driver，此函数保留为空，不再调用
    MALOGD("mas_plat_probe is not used on SPRD platform.");
    return 0;
}
...
static int finger_workerqueue_init(void)
{
    cpu_queue = create_singlethread_workqueue("ma_hs_main");
    if (!cpu_queue) {
        MALOGE("create cpu_queue failed");
        return -ENOMEM;
    }
...
int mas_probe(struct spi_device *spi) {
    ...
    // 改动：直接调用 GPIO 信息获取，传入 spi_device
    ret = mas_finger_get_gpio_info(spi);
    if(ret){
        MALOGE("mas_finger_get_gpio_info failed");
        return ret;
    }
    // 设置 GPIO 为工作状态
    ret = mas_finger_set_gpio_info(1);
    if(ret){
        MALOGE("mas_finger_set_gpio_info failed");
        return ret;
    }
    ...
    if (ret == 0) {
        ...
        ret = -ENODEV;
        goto err2;          // 释放 spi + vars
    }
#if defined(CONFIG_PRIZE_HARDWARE_INFO)
	sprintf(current_fingerprint_info.id,"%d",ret);
#endif
#elif defined TEE
	ret = 0;
#endif
   ...
   ret = finger_workerqueue_init();
   if (ret)
       goto err3;           // 释放 spi + vars（workerqueue失败）

   ret = init_interrupt();
   if (ret < 0)
       goto err3;           // 释放 workerqueue + spi + vars
   ...
   if (ret != 0) {
       ret = -ENODEV;
       goto err6;           // 释放 file_node + irq + workerqueue + spi + vars
   }

#if defined(CONFIG_PRIZE_HARDWARE_INFO)
	sprintf(current_fingerprint_info.chip,"microarray");
	strcpy(current_fingerprint_info.vendor,"MicroArray");
...
static int __init mas_init(void)
{
    ...
    MALOGF("start");
    compatible = 1;
    ret = mas_get_platform();   // 注册 SPI 驱动
    if(ret){
	   MALOGE("mas_get_platform failed");
    }
    return ret;
}
